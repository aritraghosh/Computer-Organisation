...
section .text 
 ...
 ; Initializing Flags
	 ...
	 popfd ; IP = 0x007b
;----------------------YOUR CODE--------------------
;STEP-1: setting up page directory with all entries pointing to the only page-table in 0xF000
; The page directory is in Page 0xe000 to 0xefff 

	mov ax,0x28
	mov es,ax                               ;page directory
	mov ax,0x30
	mov fs,ax                               ;page table
	mov bx,0
	L1:
		mov dword[es:bx],0xf017            ;Filling all the entries of the page directory 
		add bx,4
		cmp bx,0x1000
		jne L1
	;All pages present and pointing to the page table at f000
		
;STEP-2: setting up the page table in 0xF000
;Before this we set up the pages.Pages start at b000 and c000
	
	mov bx,0
	L2:
	   	mov dword[fs:bx],0xb017            ;All the even entries are filled with b000 and odd entries with c000 and present bit is 1
		add bx,4
		mov dword[fs:bx],0xc017
		add bx,4
		cmp bx,0x1000
		jne L2
	;Loop over.Now let us map them identically.
	; page containing Code identically mapped that enables paging - 0xA000 to 0xAFFF
	; page 1 containing Data identically mapped
    ; page 2 containing Data identically mapped
	; page containing Control Data identically mapped
	; page containing Page Directory identically mapped
	; page containing Page Table identically mapped
	; Rest of All even pages map to page at 0xb000
	; Rest of all podd pages map to page at 0xc000 
	mov dword[fs:0x28],0xa017
	mov dword[fs:0x2c],0xb017
	mov dword[fs:0x30],0xc017
	mov dword[fs:0x34],0xd017                   ;identical mapping of all pages
	mov dword[fs:0x38],0xe017
	mov dword[fs:0x3c],0xf017
	mov dword[fs:0x40],0xa017
	mov dword[fs:0x44],0xb016
    ;Filling b016.....may lead to issues
	
;STEP-3: Code to enable paging
	pushfd
	cli
	mov eax,0xe000
	mov CR3,eax
	mov eax,CR0
	BTS eax,31          ;setting 31st bit of CR0 as 1
	mov CR0,eax
	jmp foo
	foo:nop
	popfd
;STEP-4: accessing some large address and showing that actually that results in accessing only some data from b000 to cfff
;example 1
;Now we are accessing 1f000 which shall map to c000
mov ax,0x18
mov ds,ax
mov dword[ds:0x0],0xbaba
;STEP-5: jumping to a large location, and showing that actually jumped only to  the next line in this code
	
	
	
	
	;jmp L3
	JMP dword 0x20:L3-0xa000 ;jumping to the same code segment from a different register
	
	L3:
	
	mov dword[ds:0x4],0xcaca  ;to check if it actually jumped
	JMP dword 0x8:L4-0xa000   ;jumping back to original code segment
	
	L4:
;trying to access a page whose entry is such that the 'present' bit is zero and recording the page fault
;making the present bit in the  entry of the page table one using exception handler

;Step-6.1: Create a Page fault
mov ax,0x38
mov gs,ax                    ;base address 0x11000          
mov dword[gs:0x0],0xdcaf     ;Trying to change 
jmp L6
;Step-6.2:  Fill up the corresponding Entry in IDT - search for KAMA-IDT
align 0x200
;INTERRUPT SERIVCE ROUTINE
;Step-6.3: My exception handler for simply recording the specific page fault.
mov dword[fs:0x44],0xb017
INVLPG [gs:0x0]
mov dword[ds:0x8],0xface
pop eax
iretd

;Step-7: disabling paging
L6:
	mov eax,CR0
	BTR eax,31              ;changing 31st bit to 0 thereby disabling paging
	mov CR0,eax
	jmp chanHappy
;----------------------YOUR CODE--------------------
...
; Default Data Segment Descriptor of type 2, DPL = 0 ;0x10
 ...
 db 0x40
 db 0x0
;Data Segment Descriptor of type 0, random DPL and Base  ;large address 0x18
 dd 0xf0000fff , 0x409201
;Data Segment Descriptor of type 1, random DPL and Base  ;large address number 2(step 5) ;0x20
 dd 0x0000ffff , 0x4f9b01
;Data Segment Descriptor of type 2, random DPL and Base  ;page directory 0x28
 dd 0xe0000fff , 0x409200
;Data Segment Descriptor of type 3, random DPL and Base  ;page table 0x30
 dd 0xf0000fff , 0x409200
;Data Segment Descriptor of type 4, random DPL and Base  ;for step 6 0x38
 dd 0x1000ffff , 0x409201                               
;Data Segment Descriptor of type 5, random DPL and Base  ;0x40
; db 0x0
; db 0x0
; db 0x5d
; db 0xb7
...
;KAMA-IDT
dd 0x00080200,0x00008f00        ;segmentdescriptor=0x8,offest=200,DPL=0,
