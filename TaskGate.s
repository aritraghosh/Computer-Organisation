...
section .text 
 ...
 ; Initializing Flags
	 ...
	 popfd ; IP = 0x007b
;----------------------------YOUR CODE-----------------------------
; You should have set up TSS1, TSS2, TSS3, the corresponding TSS segment descriptors in GDT and also corresponding Task gates in GDT that points to these descriptors.
; At this point you are in PL0 
;Note that your Task register is loaded with 0x60 which in turn shall poin to TSS segment descriptor
; The TSS segment descriptor points to a TSS segment where the context of PL0 task will be loaded
; TO MAKE YOUR LIFE easy all these are already set in the code.
; Do a TREASURE HUNT to find it out :-)

;task zero
; Do something here so that it will be stored in TSS and will reflect you came here

;--------------------------PL-0---------------------------------------------------------------

mov EAX ,0xcafe            ;We move some value into the EAX register in PL-0(TSS of PL-0 starts at 0xda00 and EAX is starts
; Jump to PL - 1 task      ; at the 40 th byte(0x28) and so address=0xda28
;-------------------------------PL-1----------------------------------------------------------------
jmp 0x65:0x0               ;Ox60 corresponds to the Task gate for jumping from PL-0 to PL-1 ,offset by 101(LDT,requested level=PL-1)
jmp L1                     ;This is used to start executing only later when it returns back to PL-0
; use align to fix the address of PL-1 task start. This you need to have filled in TSS corresponding to 1


align 0x300                 ;0xa300
mov eax,0xface              ;We move some value into the EAX register to check it later in TSS(0xdaf8)           
;jmp 0x5c:0x0 ;In case we wanted to jump back to PL 0 from here
jmp 0x6e:0x0                ;jumping to PL-2 from PL-1 .TSS=0x68 and offset=110(0x6)
mov ax,0x2a                 ; DS of PL-2 segment=0x28 +offset=010(0x2)
mov ds,ax

align 0x300                 ;0xa600
mov eax,0xbaba              ;We move some value into the EAX register to check the value later (0xdb38+0x28=0xdb60)
mov dword[ds:0x0],0xcaba    ;We move some value into the data segment of PL-2 and try to access it from PL-3
jmp 0x57:0x0                ;Jumping from PL-2 to PL-3 (TSS=0x50+offset=111(0x7)=0x57)


align 0x300                ;0xa900 Start of PL-3 code
mov EAX,0xacfe             ;We move some value into the EAX register and try to access it from TSS later(0xdbc8)
mov dx,0x2b                ;Data segment of PL-2(0x28+011(0x3)=0x2b)  
mov fs,dx                  ;Exception raised
mov dword[fs:0x0],0xbada   ;We  write into PL-3 data segment(Chnaged in ISR to PL-3)
call 0x8c:0x0              ;We go to PL-0 execute some code and come back(0x80+100(0x4)=0x8c)
; Jump to PL 0 code
jmp 0x84:0x0               ;Jumping back from PL-3 to PL-0(TSS=0x80+offset=100(0x4))


; repeat same for PL - 2 and then to PL-3

; Now you are in PL-3

;creating exception 13 by accessing data segment in GDT of PL 2

; Come back from exception and proceed

; set up call gate and go and come back


; Use align to start the code for  exception handler for vector 13
; This should change the Privilege Level
;--------------------------------------------ISR routine-----------------------------------------
align 0x300                ;0xac00  
mov cx,0x18
mov fs,cx
mov dword[0x202c],0x40f200 ;We change it into PL-3 data segment
mov dword[fs:0x0],0xdada   ;We move something into the memory 
pop ecx                    ;Popping the error code
iretd                      ;Returning from ISR

;--------------------------FOR CALL gate PL-0 code-------------------------------------------------
align 0x300                ;0xaf00
mov cx,0x18                ;DS of PL-0 (base address=0xb000)
mov fs,cx                 
mov dword[fs:0x4],0xfafa   ;[0xb004]=Oxfafa      
retf                       ;return to PL-3 code
...
;----------------------------YOUR CODE-----------------------------
 ;Moving 0xbabe
 ;at specific location in 3rd data page to indicate successful completion of test
 chanHappy:
...
;Null and Code Descriptor with PL=0
 dd 0x0,0x0,0xa000ffff,0x4f9b00         ;Code segment of PL-0=0x8
...
;Data Segment Descriptor of type 5, random DPL and Base
 ...
 ;db 0x0
 ;db 0x0
 dd 0xc40003ff , 0x40b600	;				-------------- SS 1 --------------- 40 -------------
;Data Segment Descriptor of type 6, random DPL and Base
 ...
 ;db 0x40
 ;db 0x0
 dd 0xc80003ff , 0x40d600	;				-------------- SS 2 --------------- 48 --------------
...
;TSS Descriptor for default task - starting of the test
 ...
 ;db 0x40
 ;db 0x0
dd 0xda000067 , 0x408900	;			--------- TSS Descriptor for default task PL 0 --- 60 ----
...
; db 0x40
; db 0x0
dd 0xdad00067 , 0x40a900	;			--------- TSS Descriptor for task 1 ------- 70 ----------
;TSS [0.2] Descriptor
; db 0x67
; db 0x0
; db 0x38
...
; db 0x40
; db 0x0
dd 0xa300ffff , 0x4fbb00	;			-------------- CS 1 -------------- 78 ---------------
;TSS [0.3] Descriptor
; db 0x67
; db 0x0
; db 0xa0
...
; db 0x40
; db 0x0
dd 0xa600ffff , 0x4fdb00	;			-------------- CS 2 --------------- 80 --------------
;TSS [1.0] Descriptor
; db 0x67
; db 0x0
; db 0x8
...
; db 0x40
; db 0x0
dd 0xa900ffff , 0x4ffb00	;			-------------- CS 3 --------------- 88 --------------
;TSS [1.1] Descriptor
; db 0x67
; db 0x0
; db 0x70
...
; db 0x40
; db 0x0
dd 0xdb380067 , 0x40c900	;			-------- TSS Descriptor for task 2 ----- 90 ---------
;TSS [1.2] Descriptor
; db 0x67
; db 0x0
; db 0xd8
...
; db 0x40
; db 0x0
dd 0xdba00067 , 0x40e900	;			-------- TSS Descriptor for task 3 ----- 98 ---------
;TSS [1.3] Descriptor
; db 0x67
; db 0x0
; db 0x40
...
; db 0x40
; db 0x0
 dd 0xcc0003ff , 0x40f200	;			-------------- SS 3 --------------- a0 --------------
...
; db 0x40
; db 0x0
 dd 0xac00ffff , 0x4f9b00	;			------------- CS EXCEP PL 0 ------- a8 --------------
...
; db 0x40
; db 0x0
 dd 0xdc080067 , 0x408900	;			----- TSS Descriptor for Exception Handler PL 0 -- b0 --
...
;Data Segment Descriptor of type 7, random DPL and Base
 ...
 ;db 0x40
 ;db 0x0
dd 0x980000 , 0xe500			;------------------- Task Gate with DPL 3 for 3 to 3 switch ------ 50 -----------
 ...
 ;db 0x0
 ;db 0x0
 dd 0x600000 , 0xa500			;------------------- Task Gate with DPL 1 for 1 to 0 switch ------ 58 -----------
 ...
 ;db 0x0
 ;db 0x0
dd 0x700000 , 0xc500			;------------------- Task Gate with DPL 2 for 2 to 1 switch ------ 60 -----------
 ...
 ;db 0x0
 ;db 0x0
dd 0x900000 , 0xe500			;------------------- Task Gate with DPL 3 for 3 to 2 switch ------ 68 ----------
 ...
 ;db 0x0
 ;db 0x0
 dd 0x600000 , 0xe500			;-------------------- Task Gate with DPL 3 for 3 to 0 switch ------- 80 ---------
...
;Exception gate for type 13
 ...
 ;db 0x0
 ;db 0x0
dd 0xb00000 , 0xe500                         ;Segment selector=0xb0
...
;Empty space in third page filled with Random data
 ...
 align 0x3A00
 tss0.0:
; TSS for default code when Test starts			--------- CHANGED ------------
 dd 0x0
 dd 0x1000
 dd 0x50
 dd 0x1000
...
; TSS [0.3]			;------------ 0xdba0 --------- PL 3 task TSS ------------------
 ...
 dd 0x0
 tss1.0:
; TSS [1.0]			;-------------- 0xdc08 -------- PL 0 EXCEP TASK TSS ---------------
 dd 0x0
 dd 0x1000
 dd 0x50
 dd 0x1000
